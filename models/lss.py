...
def print_polynomial(theta: np.ndarray, precission: int = 3) -> str:
    ...
    return " + ".join([f"{np.round(t[0], precission)}*x^{i}" for i, t in enumerate(theta)])
...
def least_squares_solution(
    ...
    X_poly = X ** get_polynomial_form(polynomial_degree)

    return np.linalg.inv(X_poly @ X_poly.T) @ X_poly @ Y.reshape(1, -1).T
...
def generalised_linear_model(X: np.ndarray, T: np.ndarray) -> np.ndarray:
    ...
    """
    return np.sum(coeff * X ** degree for degree, coeff in enumerate(T))
